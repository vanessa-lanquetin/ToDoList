document.addEventListener("DOMContentLoaded", () => {
  ...
  submitButton?.addEventListener("click", function () {
    if (inputTask && inputTask instanceof HTMLInputElement) {
      const inputValue = inputTask.value.trim(); // Supprimer les espaces inutiles
      ...
      if (inputValue !== "") {
        ...
        const statusSpan = document.createElement("span"); // Créer l'élément <span> pour le statut
        statusSpan.classList.add("status"); // Ajouter une classe pour le sélectionner facilement

        const trashIcon = document.createElement("i");
        trashIcon.classList.add("fas", "fa-trash-alt");
        trashIcon.addEventListener("click", () => {
          task.remove();
          saveListToLocalStorage(); // Mettre à jour le stockage local après suppression
        });

        task.appendChild(taskText); // Ajouter le span pour le nom de la tâche
        task.appendChild(document.createTextNode(" "));
        task.appendChild(statusSpan); // Ajouter le span du statut
        task.appendChild(document.createTextNode(" "));
        ...
        task.addEventListener("click", () => {
          ...
          saveListToLocalStorage(); // Mettre à jour le stockage local après modification
        });

        list?.appendChild(task);
        saveListToLocalStorage(); // Mettre à jour le stockage local après ajout

        inputTask.value = ""; // Réinitialiser l'input après l'ajout
      }
    } else {
  ...
  function saveListToLocalStorage() {
    const tasks = Array.from(list.children).map((task) => {
      const taskText = task.querySelector("span:not(.status)"); // Sélectionner le span pour le nom de la tâche
      return {
        text: taskText?.textContent,
        isSelected: task.classList.contains("selected"),
        isFinished: task.classList.contains("finished"),
        isTodo: task.classList.contains("todo"), // Ajouter l'état "todo"
      };
    });
